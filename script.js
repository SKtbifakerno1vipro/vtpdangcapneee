...
function drawText() {
  ...
  if (Math.floor(frameNumber) == 1000) {
    opacity = 0;
  }
  ...
  if (Math.floor(frameNumber) == 1500) {
    opacity = 0;
  }
  ...
  if (Math.floor(frameNumber) == 2000) {
    opacity = 0;
  }
  ...
  if (Math.floor(frameNumber) == 2500) {
    opacity = 0;
  }
  if (frameNumber > 2500 && frameNumber < 99999) {
    ...
    if (window.innerWidth < 600) {
      drawTextWithLineBreaks(
        [
          "I love you so much {name}, more than",
      ...
      context.fillText(
        "I love you so much 'Ha oi', more than all the time and space in the universe can contain",
        canvas.width / 2,
        canvas.height / 2,
      );
    }
...
function draw() {
  ...
  if (frameNumber < 99999) {
    // Giảm tốc chạy chữ: mỗi lần tăng 0.7 để chuyển động chậm hơn.
    frameNumber += 0.7;
  }
